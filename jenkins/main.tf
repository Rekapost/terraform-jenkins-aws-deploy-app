...
output "ssh_connection_string_for_ec2" {
  value = format("%s%s", "ssh -i /home/reka/.ssh/jenkinsKVP ubuntu@", aws_instance.jenkins_ec2_instance_ip.public_ip)
}
...
output "jenkins_url" {
  value = "http://${aws_instance.jenkins_ec2_instance_ip.public_ip}:8080"
}